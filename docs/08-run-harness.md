...
로컬에서 CI와 같은 수준으로 sandbox 앱까지 실제 실행하려면 먼저 의존성을 설치한 뒤 `건너뜀 0`을 확인한다.
...
기대: `OK — 스모크 통과 (실행 2, 건너뜀 0)`. `건너뜀`이 1 이상이면 하네스 스크립트는 통과했지만 sandbox 앱 테스트는 일부 실행되지 않은 상태다.
...
주요 hook payload 경로를 한 번에 확인하려면 다음 명령을 먼저 실행한다.

```bash
node scripts/hook-smoke.mjs
```

기대: `OK — hook-smoke 통과`. 이 명령은 VS Code Preview 자동 발화를 대체하지는 않지만, Agent hooks가 받는 stdin JSON과 stdout JSON 계약이 깨지지 않았는지 빠르게 확인한다.

세션 상태 주입 — [feature_list.json](../feature_list.json) 진행 상태를 additionalContext로 출력:

```bash
echo '{"hook_event_name":"SessionStart"}' \
  | node .github/hooks/session-ready.mjs
```

PreCompact handoff — 현재 미커밋 파일 목록과 압축 전 보존 안내 출력:

```bash
echo '{}' | node .github/hooks/precompact-handoff.mjs
```

Stop hook 재진입 방지 — 이미 Stop hook이 활성인 재진입 이벤트는 차단하지 않음:

```bash
echo '{"stop_hook_active":true}' \
  | node .github/hooks/verify-done.mjs
```

기대: 세션·PreCompact hook은 JSON의 `hookSpecificOutput.additionalContext`를 출력하고, Stop 재진입 예시는 출력 없이 종료 코드 0으로 끝난다.
...
[README.md](../README.md)의 "다른 프로젝트에 적용하기" 절차로 파일을 복사한 뒤에는 대상 저장소 루트에서 아래를 확인한다.

| 확인 항목 | 기대 |
| --- | --- |
| `.github/agents/` 표시 | VS Code 에이전트 선택기에 Plan·Build·Ask 표시 |
| `.github/copilot-instructions.md` 로드 | Diagnostics에서 always-on 지침으로 표시 |
| `node scripts/harness-doctor.mjs` | 대상 저장소 구조에 맞게 통과하거나, 조정이 필요한 보호 경로를 보고 |
| `node --test tests/*.test.mjs` | hook 순수 로직 테스트 통과 |
| `node scripts/smoke.mjs` | sandbox 앱이 있으면 실행, 없거나 의존성이 없으면 출력의 `실행`·`건너뜀` 수 확인 |
| hooks Preview | 활성 환경이면 보호 경로 편집 시 PreToolUse가 차단/승인 요구 |
...
```bash
node scripts/harness-doctor.mjs && node --test tests/*.test.mjs && node scripts/hook-smoke.mjs && node scripts/smoke.mjs && echo "ALL GREEN"
```
