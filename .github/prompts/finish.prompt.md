...
## 2. 비판적 리뷰 (self-eval 회피)

검증이 통과해도 바로 완료 처리하지 않는다. 아래 항목을 **diff와 실제 출력** 기준으로 점검한다.

1. 사용자 요청과 실제 diff가 일치하는가? 요청 밖 리팩터링·메타데이터 변경이 섞였으면 분리하거나 이유를 기록한다.
2. 새 테스트·문서·hook이 주장하는 동작을 실제 명령 출력이 뒷받침하는가?
3. 보호 경로·상태 정본·handoff·문서 링크 중 누락된 동기화가 없는가?
4. 기존(pre-existing) 실패와 신규 실패가 구분됐는가?
5. 남은 위험·수동 관찰 필요 항목이 있으면 완료 선언 전에 사용자에게 명시했는가?

하나라도 불명확하면 **feature_list.json을 갱신하지 말고** 보완 또는 에스컬레이션한다.

## 3. feature_list.json 상태 갱신
...
`harness-doctor` 통과 + 링크/포맷 유효 + 비판적 리뷰 통과 + feature_list.json과 문서가 같은 커밋으로 일치. 이 조건을 **실제 명령 출력과 diff 근거로 확인**했을 때만 완료를 선언한다.
