...
node scripts/smoke.mjs            # 기대: sandbox 앱 테스트/typecheck 통과(의존성 미설치 앱은 건너뜀)
```
...
2. Copilot Chat 에이전트 선택기에서 `Plan`을 고르고 "이 작업의 영향 범위와 검증 계획을 세워줘. 아직 코드는 고치지 마."라고 요청한다.
3. 계획이 맞으면 `Build`로 바꿔 "위 계획대로 구현하고 관련 검증을 실행해줘."라고 요청한다.
4. 끝나면 `/finish`를 실행해 검증 증거, 상태 정합, 커밋 흐름을 한 번에 확인한다.

더 자세한 수동 검증은 [docs/08-run-harness.md](docs/08-run-harness.md)를 따른다.
...
- GitHub Copilot **cloud agent는 future target**이며, `handoffs`·`web`·`todo`·hooks 동작 차이가 있으므로 별도 검증 전에는 동일 동작을 보장하지 않는다(상세: [docs/07-deferred-backlog.md](docs/07-deferred-backlog.md)).
